...
void PS()
{
    ...
    // Get fog factor
        ...
        float outputValue = 0.5 * pow(clamp(2*((inputValue > 0.5) ? 1.0 - inputValue : inputValue), 0.0, 1.0), cFadeContrastPower);
        float weight = (inputValue > 0.5) ? 1.0 - outputValue : outputValue; 
        ...
        gl_FragColor = vec4(GetFog(finalColor.rgb, fogFactor), diffColor.a);
    #else
        // Ambient & per-vertex lighting
